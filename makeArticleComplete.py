...
def autoArticle(name, table):
    ...
    pyautogui.typewrite("Save money at " +name +" with data. Optimize your visit to "+ name+ " and make your life Efficient. \n"+ headerPhoto +" <h2> Save Money At " + name + '</h2> \n <ul><li>The Highest Calories Per Dollar Item is </li><li>The Highest Protein Per Dollar Item is </li><li>The Efficiency Recommendation is </li></ul> \n' + table +''' <h2>Cheapest Food</h2> \n thats still almost 2x more than we spend on homecooked food. Most menu items were around 150-250 calories per dollar, 4x more expensive than eating at home. \n Use math to eat healthy and low cost<a href="https://efficiencyiseverything.com/food/"> Efficiently</a> for $1,000/year. \n [email]''', interval=0.01)
    #pyautogui.hoykey('pagedown')
    #pyautogui.click(pyautogui.locateCenterOnScreen("wordpress_meta.png"))
...
try:
    while True:
        while (folders[folderID] != None):
            data_folder=folders[folderID]
            print (data_folder)
             



            restaurant=data_folder
            time.sleep(2)
            all_folders = os.path.join("downloads")
